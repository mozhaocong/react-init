...
export function getSysUserList(data: ObjectMap, options?: ObjectMap) {
  return axiosGet(
    'http://erp_test.admin.htwig.com' + '/api/users',
    data,
    options
  )
}
...
export const getBasicDataList = createAsyncThunk(
  ...
  async (item: any = {}, thunkAPI) => {
    ...
    const throttleName =
      'get' + type.slice(0, 1).toUpperCase() + type.slice(1) + 'Throttle'
    ...
    if (has(throttleName, throttleObject)) {
      ...
      thunkAPI.dispatch(
        businessSlice.actions.setBasicDataList({ type, data: returnData })
      )
    } else {
